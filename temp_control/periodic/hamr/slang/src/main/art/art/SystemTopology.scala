...
@sig trait Fuseable extends InfrastructureIn with InfrastructureOut {
  // no members
}
...
object InfrastructureRegistry {
  ...
  def portToTopic(portId: Art.PortId, ad: ArchitectureDescription): PortTopic = {
    ...
    if (port.mode == PortMode.EventOut || port.mode == PortMode.DataOut) {
      ...
      for (connection <- ad.connections if connection.to.id == portId) {
        return connection.from.id
      }
    }
    println("WARNING: portToTopic ERROR!")
    halt(st"impossible to convert port $portId to topic on ad $ad (in SystemTopology.portToTopic)".render)
  }

  def launchInPort(portId: Art.PortId, bundle: InPortServiceBundle, ad: ArchitectureDescription): Dequeue[DataContent] = {
    val topic: PortTopic = portToTopic(portId, ad)
    // load services
  ...
  def launchOutPort(portId: Art.PortId, bundle: OutPortServiceBundle, ad: ArchitectureDescription): Enqueue[DataContent] = {
    val topic: PortTopic = portToTopic(portId, ad)
    // load services
...
//    return enqueue // was SerializingEnqueue(enqueue, serializer) before moving deserialization to infrastructure
  ...
  def launch(serviceBundles: IS[Art.PortId, Option[PortServiceBundle]], ad: ArchitectureDescription): InfrastructureRegistry = {
    Infrastructures.resetCaches()
    val indices = serviceBundles.indices
    // launched
    val isLaunched: MSZ[B] = MSZ.create(indices.size, F)
    val inboundPortQueues: ISZ[Option[Dequeue[DataContent]]] = indices.map((portId: Art.PortId) => {
      serviceBundles(portId) match {
        case Some(bundle) => {
          bundle match {
            case (in: InPortServiceBundle) => {
              isLaunched(portId.toZ) = T
              Some(launchInPort(portId, in, ad))
            } // MUST BE FIRST CASE
    ...
    // wont work without allServices
//    def bootstrapQueueIfFusableStub(portId: Art.PortId, out: OutPortServiceBundle) = {
//      val isFuseable = out.infrastructure.isInstanceOf[Fuseable]
//      if (isFuseable) {
//        val connection: UConnection = {
//          val others: IS[Art.ConnectionId, UConnection] = ad.connections.filter((uc: UConnection) => uc.from.id == portId)
//          assert(others.size == z"1", st"fusable $portId must have from connection".render)
//          others(others.firstIndex)
//        }
//        val wasInAlreadyLaunched: B = isLaunched(connection.to.id.toZ)
//        if (!wasInAlreadyLaunched) {
//          val bundleIn = serviceBundles(connection.to.id)
//          bundleIn match {
//            case Some(inBundle: InPortServiceBundle) => launchInPort(connection.to.id, inBundle, ad)
//            case _ => println("WARNING fusable did not have InOutPortServiceBundle")
//          }
//        }
//      }
//    }
    ...
    val outboundPortQueues: ISZ[Option[Enqueue[DataContent]]] = indices.map((portId: Art.PortId) => {
      serviceBundles(portId) match {
        case Some(bundle) => {
          bundle match {
            case (out: OutPortServiceBundle) => {
//              bootstrapQueueIfFusableStub(portId, out) // bootstraps fusable queues that break during (non-system) bridge unit tests
              Some(launchOutPort(portId, out, ad))
            } // MUST BE FIRST CASE
