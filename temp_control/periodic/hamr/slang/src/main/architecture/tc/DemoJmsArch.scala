...
object DemoJmsArch {
  ...
  val services: IS[art.Art.PortId, Option[PortServiceBundle]] = {
    ...
    // fan
    services(TempControlSoftwareSystem_p_Instance_tcproc_fan.fanAck.id) = Some(jmsOut)
    services(TempControlSoftwareSystem_p_Instance_tcproc_fan.fanCmd.id) = Some(jmsIn)

    // temp sensor
    services(TempControlSoftwareSystem_p_Instance_tcproc_tempSensor.currentTemp.id) = Some(jmsOut)


    // temp control system
    services(TempControlSoftwareSystem_p_Instance_tcproc_tempControl.fanCmd.id) = Some(jmsOut)
    services(TempControlSoftwareSystem_p_Instance_tcproc_tempControl.currentTemp.id) = Some(jmsIn)

    services(TempControlSoftwareSystem_p_Instance_tcproc_tempControl.fanAck.id) = Some(jmsIn)
    services(TempControlSoftwareSystem_p_Instance_tcproc_tempControl.setPoint.id) = Some(jmsIn)

    // operator interface
    services(TempControlSoftwareSystem_p_Instance_tcproc_operatorInterface.setPoint.id) = Some(jmsOut)
    services(TempControlSoftwareSystem_p_Instance_tcproc_operatorInterface.currentTemp.id) = Some(jmsIn)
